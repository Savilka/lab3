	...
	if(inp_str.size() != 0){
		...
		switch(first_sym_type){
			...
			case 1:{ //Если первый символ - константа
				...
				for(i = 0; i < inp_str.size() && !local_error; i++){
					local_error = !((numbs.is_element_in_table(inp_str[i]) || admis_chars.is_element_in_table(inp_str[i]) ||
						inp_str[i] == '.' || inp_str[i] == '\'') && inp_str[i] != ';'); //это либо число, либо точка либо кавыка
				}
				int start = i - 1;
				if(local_error)
					start--;
				...
				if (inp_str[inp_str.size() - 1] == ';') {
					continue_str = inp_str[inp_str.size() - 1];
					inp_str = inp_str.substr(0, inp_str.size() - 1);
				}
				...
				{
				case '.':
					points_n = inp_str.find_last_of('.') - inp_str.find_first_of('.'); //количество вхождения точек
					if (points_n != 0) {
						local_error = true;
						errors_s << "Много точек)" << endl;
					}
					else {
						string substr = inp_str.substr(1);
						if (mystoi(substr)) {
							inp_str = inp_str.substr(1);
							isFloat = true;
						}
						else {
							errors_s << "Неправильная константа с плавающей точкой" << endl;
							local_error = true;
						}
					}
					break;
				case '\'':
					if (inp_str[0] == '\'' && inp_str[2] == '\'' && inp_str.size() == 3 && (isalpha(inp_str[1]) || isdigit(inp_str[1]))) {
						inp_str = inp_str.substr(1, 1);
						isChar = true;
					}
					else {
						errors_s << "Неправильная символьная константа" << endl;
						local_error = true;
					}
				...
				default:
					...
					if (mystoi(inp_str));
						...
						errors_s << "Неправильная константа" << endl;
						local_error = true;
					}
...
int translator::check_symbol(char sym){
	if(admis_chars.is_element_in_table(sym))
		return 0;
	if(numbs.is_element_in_table(sym) || sym == '.' || sym == '\'')
		return 1;
	...
	while(!local_error){
		...
		if(find_terminal){ //если получаем то, что ожидали то обрабатываем это
			...
			if(parsing_table[cur_row].put_in_stack)
				parsing_stack.push(cur_row+1); //если надо получить в стек - кладём
			...
			if(parsing_table[cur_row].error){ //если можем судить что уже ошибка, то возвращаем её
				...
				do{
					for(int i = 0 ; i < parsing_table[cur_row].terminal.size(); i++){
						parse_error_f << parsing_table[cur_row].terminal[i] << " ";
					}
					cur_row--;
		...
		if (escape) break;
		if (parse_token_f.eof()) escape = true;
